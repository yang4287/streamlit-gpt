...
def set_seesion_id():
    ...
    cookie_manager.set("session_id", token_with_date, expires_at = datetime.now() + timedelta(days=365))


if not cookie_manager.get("session_id"):
    set_seesion_id()

selected_llm_model = st.sidebar.selectbox("選擇模型", ["gpt-3.5-turbo", "gpt-4-turbo"])

# === 設定Langchain 架構 ===
llm = ChatOpenAI(model=selected_llm_model,api_key = st.secrets["OPENAI_API_KEY"])
